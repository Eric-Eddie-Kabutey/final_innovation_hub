...
const cardData = [
  { text: "You", bgColor: "bg-gray-200", textColor: "text-gray-800" },
  { text: "Are", bgColor: "bg-red-700", textColor: "text-white" },
  { text: "Next", bgColor: "bg-[#53565a]", textColor: "text-white" },
];
      ...
      <section className="max-container 2xl:w-[85%] md:w-[95%] w-[90%] mx-auto bg-white flex flex-col gap-6">
        <div className="md:max-w-[60%] mx-auto flex flex-col items-center gap-2">
          ...
          <Typography typo="header-2-medium" className="text-center">
            Join a network of over 21,000+ TEF Alumni driving impact across{" "}
        ...
        <div className="w-[80%] mx-auto py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
          ...
          `}
            >
              <span
                className={`${card.textColor} text-4xl lg:text-5xl font-extrabold`}
              >
                {card.text}
              </span>
            </div>
        ...
        <div className="md:w-[20%] w-[40%] mx-auto py-8 grid grid-cols-4 gap-6">
          {countryFlags.map((data, index) => (
            <div key={`Alumni ${index + 1}`} className="w-full h-fit">
              <Image src={data} alt="Alumni image" className="w-full h-full" />
            </div>
          ))}
        </div>
