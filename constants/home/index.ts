import amen_benson from "@/public/assets/images/alumni_images/amen_benson.webp";
import daniel_narh from "@/public/assets/images/alumni_images/daniel_narh.webp";
import ofili_img from "@/public/assets/images/alumni_images/ofili_img.webp";
import alumni4 from "@/public/assets/images/alumni_images/4.png";
import alumni5 from "@/public/assets/images/alumni_images/5.png";
import alumni6 from "@/public/assets/images/alumni_images/6.png";

import ghanaFlag from "@/public/assets/images/country_flags/ghana_flag.png"
import gambiaFlag from "@/public/assets/images/country_flags/gambia.png"
import kenyaFlag from "@/public/assets/images/country_flags/kenya.png"
import ugandaFlag from "@/public/assets/images/country_flags/uganda.png"

export const alumniImgs = [amen_benson, daniel_narh, ofili_img, alumni4, alumni5, alumni6]

export const countryFlags = [ghanaFlag, gambiaFlag, kenyaFlag, ugandaFlag];

export const faqs = [
    {
        id: 1,
        question: "What programmes are open for application in January 2025?",
        answer: `TEF Flagship Entrepreneurship Programme: Open to all entrepreneurs across Africa with business ideas or existing businesses. We encourage applications from businesses leveraging AI as well as green businesses. For existing businesses, business registration must not exceed five years at the time of application. Applicants must be at least 18 years old.`,
    },
    {
        id: 2,
        question: "What are the eligibility criteria for businesses?",
        answer: `All programmes are open to businesses that are between 0 and 5 years old at the time of application. Businesses older than 5 years are not eligible. However, the Aguka Ideation Programme is exclusive to Rwandan entrepreneurs with businesses in the idea stage.`,
    },
    {
        id: 3,
        question: "How do I apply?",
        answer: `Applications can be submitted on www.tefconnect.com, and will be open from 1st of January to the midnight on the 1st of March 2025. Ensure to complete your application well ahead of the deadline.`,
    },
    {
        id: 4,
        question: "Can I apply to more than one Entrepreneurship programme?",
        answer: `No, applicants are advised to apply to only one programme. We encourage you to carefully review the programme descriptions and select the one that best suits your entrepreneurial and business goals.`,
    },
    {
        id: 5,
        question: "How do I know which programme is right for me?",
        answer: `We recommend reviewing the programme descriptions to determine which one aligns best with your business stage, demographic, and focus area.`,
    },
]